...
const DAY = 86400; // 24 hours cache window
...
async function fetchOddsForLeague(oddsKey: string, leagueName: string) {
  ...
  try {
    // Cached for 24 hours at the fetch level
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      tips,
      note: allFixtures.length === 0 ? "No fixtures found today in the tracked leagues." : undefined,
    });
  } catch (err) {
